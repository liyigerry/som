...
raw = pickle.load(open('../../raw_database/identity.pkl', 'rb'))
dataset = []
for mol in raw:
    g = mol[0]
    y = mol[1]
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.conv1 = GATConv(10, 512,  heads=8)
        self.conv2 = GATConv(8 * 512, 512, heads=8)
        self.conv3 = GATConv(8 * 512, 512, heads=1)
...
def ntimecrossvalidation(n=5):
    ...
    pickle.dump(all_records, open('./new_all_records_identity.pkl', 'wb'))

args = {
    'lr': 0.01,
    'epoch': 500,
    'seed': 42,
    'save_path': './identitymodel/model',
    'momentum':0.9,
    'weight_decay': 1e-5,
    'pos_weight': 2
}
...
with open('./time.txt', 'a') as f:
    f.write('new identity spend time ' + str(endtime) + '\n')
